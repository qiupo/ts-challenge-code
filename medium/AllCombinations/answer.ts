...
type getFirst<S> = S extends `${infer First}${any}` ? First : '';
type AllCombinations<S, C extends string | number | bigint | boolean | null | undefined = getFirst<S>> =
    C extends '' ? '' :
    S extends `${infer First}${C}${infer Rest}`
    ? C | 
    `${C}${AllCombinations<`${First}${Rest}`>}` |
    `${AllCombinations<`${First}${Rest}`>}${C}` |
    AllCombinations<`${First}${Rest}`> |
    AllCombinations<S, getFirst<Rest>> : ''
